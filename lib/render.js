...
function render (app, container, opts) {
  ...
  function renderEntity (entity) {
    ...
    var result = fn(entity.context, triggerEvent(entity))
  ...
  /**
   * Trigger an event on the current component
   *
   * @param {Entity} entity
   *
   * @return {Function} A function curried for triggering an event on the entity
   */
  ...
  function triggerEvent (entity) {
    return function () {
      ...
      return function () {
        ...
        // Make old tests / setup work for now
        if('object' === typeof args[0])
          return updateEntityState(entity, args[0])

        var newState = trigger('update', entity, [entity.context].concat(args))

        if (newState) {
          updateEntityState(entity, newState)
        } else {
          bubbleEvent(entity, args)
        }
      }
    }
  }
  ...
  /**
   * Bubble an event up the component tree
   *
   * @param {Entity} entity
   * @param {Array} args to pass to update
   *
   * @return {void}
   */
  function bubbleEvent (entity, args) {
    if (! entity) return

    var ctx = entity.context
    var fn = ctx && ctx.props && ctx.props[handlerName(args[0])]

    if (fn) {
      fn.apply(null, args)
    } else if (entity.ownerId) {
      bubbleEvent(components[entity.ownerId], args)
    }
  }

  /**
   * Create an event handler name from an event name (e.g. 'click' => 'onClick')
   *
   * @param {String} event name
   *
   * @return {String} event handler name
   */
  ...
  function flushMountQueue () {
    while (mountQueue.length > 0) {
      ...
      var entity = entities[entityId]
      triggerUpdate('afterRender', entity, [entity.context, entity.nativeElement])
      triggerUpdate('afterMount', entity, [entity.context, entity.nativeElement, triggerEvent(entity)])
    }
  }
  ...
  function updateEntity (entityId) {
    ...
    trigger('afterUpdate', entity, [entity.context, previousProps, previousState, triggerEvent(entity)])
  }
  ...
  function triggerUpdate (name, entity, args) {
    var update = triggerEvent(entity)
    args.push(update)
  ...
  function addEvent (entityId, path, eventType, fn) {
    keypath.set(handlers, [entityId, path, eventType], function (e) {
      var entity = entities[entityId]
      if (entity) {
        fn.call(null, e, entity.context, triggerEvent(entity))
      } else {
        fn.call(null, e)
      }
    })
